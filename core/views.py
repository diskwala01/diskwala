...
class AnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        # Last 30 Days Chart (Views + Downloads combined)
...
def admin_users(request):
    users = User.objects.all().annotate(
        file_count=Count('files')  # files = related_name in UserFile model
    )
    ...
    data = list(users.values(
        ...
        'is_active',
        'created_at',
        'file_count'  # Ab yeh allowed hai kyuki annotate ke baad values() mein include kiya
    ))
...
def admin_reject_withdrawal(request, pk):
    w = get_object_or__404(Withdrawal, pk=pk)
    w.status = 'rejected'
    w.processed_at = timezone.now()
    w.user.pending_earnings += w.amount
    w.user.save()
...
# ========================
# PUBLIC ADMOB IDs API
# ========================
@api_view(['GET'])
@permission_classes([AllowAny])
def get_admob_ids(request):
    """
    Flutter app is endpoint se AdMob IDs fetch karega
    """
    settings = SiteSettings.get_settings()
